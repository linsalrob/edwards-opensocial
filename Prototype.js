...
content.stripScripts());
 ...
 wrap: function(element, wrapper, attributes) {
   ...
   else if (Object.isString(wrapper)) wrapper = new Element(wrapper,
attributes);
   ...
   pseudos: {
     ...
     'not': function(m) {
       ...
       while (e && le != e && (/\S/).test(e)) {
         ...
         for (var i in p) {
           if (m = e.match(p[i])) {
             v = Object.isFunction(x[i]) ? x[i](m) : new
...
+ 1) ", m);
     ...
     'only-of-type':   function(m) {
       var p = Selector.xpath.pseudos; return p['first-of-type'](m) +
p['last-of-type'](m);
     },
...
c); c = false;').evaluate(m);
   ...
   pseudo: function(m) {
     ...
     return new Template('n = h.pseudo(n, "#{1}", "#{6}", r, c); c =
false;').evaluate(m);
   },
...
matches[5] || matches[6]);
 ...
 handlers: {
   ...
   index: function(parentNode, reverse, ofType) {
     ...
     if (reverse) {
       for (var nodes = parentNode.childNodes, i = nodes.length - 1,
j = 1; i >= 0; i--) {
         var node = nodes[i];
...
node._countedByPrototype)) node.nodeIndex = j++;
       ...
       for (var i = 0, j = 1, nodes = parentNode.childNodes; node =
nodes[i]; i++)
         if (node.nodeType == 1 && (!ofType ||
node._countedByPrototype)) node.nodeIndex = j++;
     }
...
var $continue = new Error('"throw $continue" is deprecated, use
"return" instead');
